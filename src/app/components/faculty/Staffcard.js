...
function Staffcard({ name, designation, image, email }) {
  return (
    <div className="w-[320px] mx-4 bg-white rounded-xl shadow-md hover:shadow-xl overflow-hidden transition-all duration-300 border border-gray-100 flex flex-col">
      ...
      {/* Content section */}
      <div className="text-black px-6 py-5 text-center flex flex-col flex-1">
        <h3
          className="text-xl font-bold text-gray-800 mb-2 leading-snug line-clamp-2"
          title={name}
        >
          {name}
        </h3>
        ...
        <div className="flex items-center justify-center py-2 px-4 bg-gray-50 rounded-lg mb-4 border-l-4 border-red-700">
          <FontAwesomeIcon icon={faIdCard} className="w-4 h-4 text-red-700 mr-2 shrink-0" />
          <h4 className="text-md font-medium text-gray-700 truncate">{designation}</h4>
        </div>

        {email && (
          <a
            href={`mailto:${email}`}
            className="flex items-center justify-center gap-2 mt-auto text-sm text-gray-700 hover:text-blue-600 transition-all duration-300"
          >
            <FontAwesomeIcon
              icon={faEnvelope}
