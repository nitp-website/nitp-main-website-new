...
const Sidebar = ({ summary, Data }) => {
  ...
  const { ipr } = facultyData;
  const [qrCode, setQrCode] = useState("");
  const [error, setError] = useState(null);

  // ipr count: use summary counts if available, else filter array (legacy)
  const iprCount = counts.ipr !== undefined
    ? Number(counts.ipr)
    : (ipr?.filter(i => i.grant_date || i.registration_date || i.publication_date)?.length || 0);
  ...
  const dataSections = [
    ...
    { label: "Intellectual Property Rights", count: iprCount,                        bgColor: "bg-orange-500" },
  ...
  const aboutMeData = facultyData.about_me || [];
    ...
    <div className="h-fit w-full flex-col shadow-lg text-black rounded-md">
      <div className="h-fit w-full rounded-md">
          ...
          {linkedin && (
            <div className="absolute right-0 top-1/2 -translate-y-1/2 w-16 h-16 flex flex-col gap-2 bg-red-600 items-center justify-center rounded-l-lg px-2">
              <Link href={linkedin} className="flex flex-col items-center">
                <img src="/linkedin.svg" className="bg-black rounded-full h-8 w-8" />
              </Link>
            </div>
          )}
        </div>
        ...
        <div className="md:hidden flex flex-col justify-between gap-4 p-6 text-black rounded-lg">
          <div className="flex flex-col min-[475px]:flex-row justify-between bg-white gap-4">
            ...
            <div className="flex flex-col items-center">
              ...
              {cv && (
                <a href={cv} className="text-xs text-center mt-4 px-4 py-2 bg-blue-600 text-white font-bold rounded-lg hover:bg-blue-700 transition" download>
                  Download CV
                </a>
              )}
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mt-6 justify-center items-center">
            <div className="flex gap-4 flex-wrap justify-center">
              {dataSections.slice(0, 2).map((section, index) => (
                <div key={index} className={`w-24 h-24 flex text-center items-center justify-center text-white font-bold ${section.bgColor} rounded-lg`}>
                  <h3 className="text-sm"><span className="text-4xl">{section.count}</span><br />{section.label}</h3>
                </div>
              ))}
            </div>
            <div className="flex gap-4 flex-wrap justify-center">
        ...
        <div className="flex p-2 justify-evenly gap-2 h-fit text-black">
          ...
          {scopus && (
            <div className="w-16 h-16 bg-slate-200 flex justify-center rounded-md items-center">
              <Link href={scopus} className="flex flex-col items-center">
                <img src="/scopus.svg" className="bg-red-600 rounded-full h-8 w-8" />
                <h3>scopus</h3>
              </Link>
            </div>
          )}
          {google_scholar && (
            <div className="w-16 h-16 bg-slate-200 flex justify-center rounded-md items-center">
              <Link href={google_scholar} className="flex flex-col items-center">
                <img src="/googleScholar.svg" className="bg-blue-600 rounded-full h-8 w-8" />
                <h3>Scholar</h3>
              </Link>
            </div>
          )}
          {orcid && (
            <div className="w-16 h-16 bg-slate-200 flex justify-center rounded-md items-center">
              <Link href={orcid} className="flex flex-col items-center">
                <img src="/orcid.svg" className="bg-yellow-600 rounded-full h-8 w-8" />
                <h3>ORCID</h3>
              </Link>
            </div>
          )}
      ...
      <div className="w-full px-4">
        {aboutMeData.length > 0 && aboutMeData[0]?.content && (
          <div className="mt-6 p-3 bg-white rounded-md shadow-sm">
            <h2 className="text-2xl font-bold border-b pb-2 uppercase">About</h2>
            <p className="text-gray-700 mt-2 text-justify">{aboutMeData[0].content}</p>
          </div>
        )}
        {research_interest && (
          <div className="mt-6 p-3 bg-white rounded-md shadow-sm">
            <h2 className="text-2xl font-bold border-b pb-2 uppercase">Expertise</h2>
