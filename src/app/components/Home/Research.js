...
export default function Research() {
  ...
  const [patentsError, setPatentsError] = useState(false);
  ...
  const ErrorMessage = () => (
    <div className="flex justify-center items-center py-8">
      <div className="text-center">
        ...
        <p className="text-red-500 pt-4">Sorry, failed to fetch data.</p>
      </div>
    </div>
  );

  // Safe sort function that handles null/undefined arrays
  const safeSortProjects = (projects) => {
    if (!Array.isArray(projects)) return [];
    return [...projects].sort((a, b) => new Date(b.end_date) - new Date(a.end_date)).slice(0, 30);
  };

  const safeSortPublications = (publications) => {
    if (!Array.isArray(publications)) return [];
    return [...publications].sort((a, b) => b.year - a.year).slice(0, 30);
  };
            ...
            <div className="p-4 h-[500px] overflow-hidden relative">
              {recentPublications.length === 0 ? (
                ...
                <div className="space-y-4">
                  {safeSortPublications(recentPublications).map((pub, index) => (
          ...
          <div className="w-full border border-primary shadow-lg hover:shadow-xl transition-shadow duration-300 rounded-lg">
            ...
            <div className="p-4 h-[500px] overflow-hidden relative">
              {recentProjects.length === 0 ? (
                ...
                <div className="space-y-4">
                  {safeSortProjects(recentProjects).map((project, index) => (
