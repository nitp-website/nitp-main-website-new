...
const Home = () => {
  ...
  const fetchPhd = async () => {
    try {
      setLoading(true);
      const api = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/faculty?type=hss`;
  ...
  const hasStaff =
    staffData.find((dept) => dept.department === "Humanities")?.staff.length >
    0;
      ...
      {staff && hasStaff && (
        <div className="flex flex-col">
          ...
          <div className="flex flex-wrap justify-center gap-10 p-5 my-2 text-black">
            {staffData
              .find((dept) => dept.department === "Humanities")
