...
const Home = () => {
  ...
  const fetchPhd = async () => {
    try {
      setLoading(true);
      const api = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/faculty?type=ece`;
  ...
  const hasStaff = staffData?.find((dept) => dept.department === "ECE")?.staff?.length > 0 || false;
