...
function Page() {
    const eeTimeTable = timeTableData.find(
        (item) => item.name === 'Electrical Engineering'
    );
    const timeTableLink = eeTimeTable ? eeTimeTable.url : '#';

    const data = eeTimeTable
        ? [{ name: eeTimeTable.name, link: eeTimeTable.url }]
        : [];
