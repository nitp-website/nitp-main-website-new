...
import staffData from "../../staffArch";
...
const Home = () => {
  ...
  const fetchphd = async () => {
    setloading(true);
    const api = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/faculty/arch`;
...
const Staff = staffData.find((dept) => dept.department === "Architecture and Planning")?.staff || [];
useEffect(() => {
    fetchphd();
}, [phd]);
