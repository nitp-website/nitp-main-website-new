...
const Home = () => {
  ...
  const fetchPhd = async () => {
    try {
      setLoading(true);
      const api = `${process.env.NEXT_PUBLIC_BACKEND_API_URL}/api/faculty?type=ce`;
  ...
  const hasStaff =
    staffData.find((dept) => dept.department === "Civil")?.staff.length > 0;
    ...
    <div className="flex flex-col px-3 py-10 max-sm:p-4 text-black">
      ...
      {faculty && hasFaculty && (
        <div className="flex flex-col">
          ...
          <FacultyList url={"/Department/CE/faculty"} branch={"ce"} />
        </div>
      )}
      ...
      {staff && hasStaff && (
        <div className="flex flex-col">
          ...
          <div className="flex flex-wrap justify-center gap-10 p-5 my-2 text-black">
            {staffData
              .find((dept) => dept.department === "Civil")
